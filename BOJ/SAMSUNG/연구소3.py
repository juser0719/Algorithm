"""
중요한건 비활성 바이러스까지 다 확산 시킬 필요없이 0인 지역만 다 오염시키면 그만임.
"""
...
def infection(virus):
    ...
    while virus:  # 바이러스를 퍼뜨리는 작업
        # 모든바이러스를 퍼뜨리면 break( 비활성화 된 바이러스는 굳이 활성화 시킬 필요 없다)
        if virus_cnt == safe_area:
            break
        x, y = virus.popleft()
        for k in range(4):
            nx = x + dx[k]
...
safe_area = 0  # 바이러스의 개수 -> 나중에 바이러스 퍼뜨릴 때 바이러스가 "이미 다 퍼져있음에도" 비활성화된 바이러스로 퍼져가려는 것을 막기 위해!
